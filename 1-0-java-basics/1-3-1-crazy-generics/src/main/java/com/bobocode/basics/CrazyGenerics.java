...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T targetEntity) {
            return entities.stream()
                    .map(BaseEntity::getUuid)
                    .filter(id -> id.equals(targetEntity.getUuid()))
                    .count() > 1;
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            var max = elements.iterator().next();
            for (T el : elements) {
...
//            Copied solution from Collections.swap(...). Option with removing <?> also works
            final List list = elements;
            var iElement = list.get(i);
            var jElement = list.get(j);
            list.set(i, jElement);
            list.set(j, iElement);
        }

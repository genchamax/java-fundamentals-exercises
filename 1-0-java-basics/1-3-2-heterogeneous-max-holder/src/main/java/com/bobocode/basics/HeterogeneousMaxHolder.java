...
public class HeterogeneousMaxHolder {
    ...
    public <T extends Comparable<? super T>> T put(Class<T> key, T value) {
        ...
        var current = (Comparable<T>) container.get(key);
        if (Objects.isNull(current) || current.compareTo(value) < 0) {
    ...
    public <T> T put(Class<T> key, T value, Comparator<T> comparator) {
        ...
        var current = container.get(key);
        if (Objects.isNull(current) || comparator.compare(value, (T) current) > 0) {
            container.put(key, value);
            return (T) current;
        }
    ...
    public <T extends Comparable<? super T>> T getMax(Class<T> key) {
